...
const chromeCopy = {
  "zh-CN": {
    ...
    requiredCount: "必要项目",
    optionalCount: "可选插件",
    previewFromBuilder: "实时预览正在使用构建器表单生成的默认配置。",
    previewFromJson: "实时预览正在使用导出窗口中的 JSON 配置。",
    validDefault: "配置结构有效，实时预览可用。",
  ...
  "zh-TW": {
    ...
    requiredCount: "必要項目",
    optionalCount: "可選外掛",
    previewFromBuilder: "即時預覽正在使用建構器表單產生的預設配置。",
    previewFromJson: "即時預覽正在使用匯出視窗中的 JSON 配置。",
    validDefault: "配置結構有效，即時預覽可用。",
  ...
  en: {
    documentTitle: "Privacy Banner Builder",
    ...
    valid: "Valid",
    invalid: "Needs fixes",
    requiredCount: "Required services",
    optionalCount: "Optional plugins",
    previewFromBuilder: "Live preview is using the default config generated by the builder form.",
    previewFromJson: "Live preview is using the JSON config from the export dialog.",
    validDefault: "Config structure is valid and live preview is available.",
...
function validateConfig(config, parseError = null) {
  const text = chrome();
  const errors = [];
  if (parseError) errors.push(text.invalidJson);
  if (!config?.ui?.[state.activeLang]) errors.push(text.missingUi);
  if (!Array.isArray(config?.requiredServices) || config.requiredServices.length === 0) errors.push(text.missingRequired);
  ...
  return {
    ok: errors.length === 0,
    errors,
    requiredCount: Array.isArray(config?.requiredServices) ? config.requiredServices.length : 0,
    optionalCount: Array.isArray(config?.plugins) ? config.plugins.length : 0
  };
}

function renderValidation(result, sourceKey) {
  const text = chrome();
  validationStatus.textContent = result.ok ? text.valid : text.invalid;
  validationStatus.classList.toggle("is-valid", result.ok);
  validationRequiredCount.textContent = String(result.requiredCount);
  validationOptionalCount.textContent = String(result.optionalCount);
  validationSource.textContent = text[sourceKey];
  validationList.innerHTML = result.ok
    ? `<li>${escapeHtml(text.validDefault)}</li>`
...
configJson.addEventListener("input", () => {
  try {
    ...
    if (result.ok) renderPreview(config);
    renderValidation(result, "previewFromJson");
  } catch (error) {
    renderValidation(validateConfig(buildConfig(), error), "previewFromJson");
  }
});
